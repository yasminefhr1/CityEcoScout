...
# Fonction pour filtrer uniquement les restaurants avec des photos
...
def save_to_csv(results, filename="restaurants_with_photos.csv"):
...
# Script principal
def main():
    query = "sustainable restaurant"  # Mot-clé pour rechercher des restaurants
    ...
    for i, (lat, lng) in enumerate(grid_points):
        ...
        # Filtrer les restaurants avec photos uniquement
        filtered_results = filter_places_with_photos(results)
        all_results.extend(filtered_results)
        print(f"{len(filtered_results)} restaurants avec photos trouvés à {location}.")

        # Pause pour éviter de dépasser les quotas de l'API
        time.sleep(1)

    # Sauvegarde des résultats dans un fichier CSV
    save_to_csv(all_results, "restaurants_with_photos.csv")
    print(f"Tous les restaurants avec photos ont été sauvegardés dans 'restaurants_with_photos.csv'.")
