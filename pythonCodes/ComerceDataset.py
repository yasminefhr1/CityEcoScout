...
# Fonction pour filtrer uniquement les commerces avec des photos
def filter_places_with_photos(results):
    filtered_results = []
    seen_place_ids = set()  # Ensemble pour suivre les `place_id` déjà vus
    for place in results:
        place_id = place.get("place_id")
        if place_id and place_id not in seen_place_ids:  # Vérifie si le commerce a déjà été ajouté
            if "photos" in place:  # Vérifie si des photos sont disponibles
                ...
                if photo_reference:
                    ...
                    seen_place_ids.add(place_id)  # Ajoute le `place_id` à l'ensemble pour éviter les doublons
    return filtered_results

def save_to_csv(results, filename="commerces_with_photos.csv"):
...
# Script principal
def main():
    query = "sustainable commerce"  # Mot-clé pour rechercher des commerces
    ...
    for i, (lat, lng) in enumerate(grid_points):
        ...
        # Filtrer les commerces avec photos uniquement
        filtered_results = filter_places_with_photos(results)
        all_results.extend(filtered_results)
        print(f"{len(filtered_results)} commerces avec photos trouvés à {location}.")

        # Pause pour éviter de dépasser les quotas de l'API
        time.sleep(1)

    # Sauvegarde des résultats dans un fichier CSV
    save_to_csv(all_results, "commerces_with_photos.csv")
    print(f"Tous les commerces avec photos ont été sauvegardés dans 'commerces_with_photos.csv'.")
